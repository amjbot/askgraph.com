...
struct
  ...
  let buildExpressionParser operators simpleExpr =
    let makeParser term ops =
      ...
      let (ambigR,ambigL,ambigN,ambigPre,ambigPost) = 
        ...
        in
          (ambiguous "right" raOp, ambiguous "left" laOp, ambiguous "non" naOp,
           ambiguous "prefix" preOp, ambiguous "postfix" postOp)
      in

      let id x = x in

      let rec rpreP st = (
        (preOp >>= fun f1 -> rpreP >>= fun f2 -> return (fun x -> f1 (f2 x)))
        <|> return id
      ) st

      and rpostP st = (
        (postOp >>= fun f1 -> rpostP >>= fun f2 -> return (fun x -> f2 (f1 x)))
        <|> return id
      ) st
